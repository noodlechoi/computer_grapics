...
private:
	...
	float m_camera_pitch{ -30.0f }; // x축
	float m_camera_yaw{ 0.0f }; // y축
	// roll 대신 up vector로 컨트롤

	glm::vec3 m_camera_pos{ glm::vec3(0.0f, 5.0f, 10.0f) };
	...
	// light
	glm::vec3 radius = { glm::vec3(5.0f, 0.0f, 0.0f) };

	//glm::vec3 m_light_pos{ glm::vec3(0.0f, 0.0f, 0.0f) };
	glm::vec3 m_light_color{ glm::vec3(1.0f, 1.0f, 1.0f) };
	glm::vec3 m_object_color{ glm::vec3(0.0f, 1.0f, 1.0f) };
	float m_ambient_strength{ 0.2f };
